...
const cyberColors = {
  ...
  accent: 'text-[#d0d0f0]',
  glass: 'bg-[#0f0f1fcc] backdrop-blur-lg border border-[#00ffff33]'
};

// Custom Arrow Components
const NextArrow = ({ onClick }: { onClick: () => void }) => (
  <div className={`absolute right-0 top-1/2 transform -translate-y-1/2 z-10 cursor-pointer text-3xl ${cyberColors.primary} hover:text-[#ff00ff]`} onClick={onClick}>
    &gt;
  </div>
);

const PrevArrow = ({ onClick }: { onClick: () => void }) => (
  <div className={`absolute left-0 top-1/2 transform -translate-y-1/2 z-10 cursor-pointer text-3xl ${cyberColors.primary} hover:text-[#ff00ff]`} onClick={onClick}>
    &lt;
  </div>
);
...
function CompanyCarousel({ companies, onSelectCompany }: { 
  ...
  return (
    <div className={`${cyberColors.glass} p-4 rounded-lg shadow-cyber max-w-4xl mx-auto mb-8 relative overflow-hidden`}>
      <Slider {...settings}>
        {companies.map((company) => (
          <div
            key={company.name}
          ...
          >
            <img 
              src={company.logo} 
              alt={company.name} 
              className="w-32 h-32 object-contain filter drop-shadow-neon transition-transform duration-300 group-hover:scale-110" 
            />
            <p className={`text-center mt-4 font-bold ${cyberColors.primary} text-xl group-hover:text-[#ff00ff] transition-colors`}>
              {company.name}
            </p>
          </div>
        ))}
...
// CompanyDetails Component
function CompanyDetails({ company }: { company: CompanyProcess }) {
  return (
    <div className={`${cyberColors.glass} rounded-lg p-6 mt-6 max-w-4xl mx-auto relative overflow-hidden`}>
      <div className="flex items-center gap-4 mb-6">
        <img 
          src={company.logo} 
          alt={`${company.name} logo`} 
          className="w-12 h-12 object-contain filter drop-shadow-neon" 
        />
        <h2 className={`text-2xl font-bold ${cyberColors.primary}`}>{company.name}</h2>
      </div>
      <p className={`${cyberColors.accent} mb-4`}>{company.description}</p>
      <div className={`${cyberColors.glass} p-4 rounded-lg mb-6`}>
        <p className={`${cyberColors.secondary} font-bold`}>Average Timeline: {company.averageTimeline}</p>
      </div>
      <div className="mb-6">
        <h3 className={`text-lg font-bold ${cyberColors.primary} mb-4`}>Eligibility Criteria:</h3>
        <ul className="space-y-2">
          {company.eligibility.map((criteria, index) => (
            <li 
              key={index}
              className={`${cyberColors.accent} flex items-center before:content-['▹'] before:mr-2 before:${cyberColors.secondary}`}
            >
              {criteria}
            </li>
          ))}
      ...
      <div className="mb-6">
        <h3 className={`text-lg font-bold ${cyberColors.primary} mb-4`}>Recruitment Process:</h3>
        <div className="space-y-4">
          {company.steps.map((step, index) => {
            const Icon = step.icon;
            return (
              <div key={index} className="flex items-start gap-4 group">
                <div className={`flex-shrink-0 w-8 h-8 flex items-center justify-center rounded-full ${cyberColors.glass} border border-[#00ffff33]`}>
                  <Icon className={`w-5 h-5 ${cyberColors.secondary}`} />
                </div>
                <div>
                  <h4 className={`font-bold ${cyberColors.primary}`}>{step.title}</h4>
                  <p className={`${cyberColors.accent}`}>{step.description}</p>
                </div>
              </div>
            );
          })}
      ...
      <div className={`${cyberColors.glass} p-4 rounded-lg`}>
        <h3 className={`text-lg font-bold ${cyberColors.primary} mb-2`}>Additional Information:</h3>
        <p className={`${cyberColors.accent}`}>{company.additionalInfo}</p>
      </div>
    </div>
  );
}
...
function App() {
  ...
  return (
    <div className={`min-h-screen ${cyberColors.background} py-12 font-space-grotesk`}>
      <style jsx global>{`
        .shadow-cyber {
      ...
      <div className="container mx-auto px-4">
        <div className="text-center mb-8">
          <h1 className={`text-4xl font-bold ${cyberColors.primary} mb-4`}>
            TECH COMPANY RECRUITMENT PROCESSES
          </h1>
          <p className={`text-xl ${cyberColors.secondary}`}>
            EXPLORE RECRUITMENT DETAILS FOR TOP TECH COMPANIES
          </p>
        </div>
        <CompanyCarousel companies={companies} onSelectCompany={setSelectedCompany} />
        {selectedCompany && <CompanyDetails company={selectedCompany} />}
