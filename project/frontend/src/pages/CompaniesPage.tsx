...
const NextArrow = ({ onClick }: { onClick: () => void }) => (
  <div
    className="absolute right-4 top-1/2 -translate-y-1/2 z-10 cursor-pointer p-2
               rounded-full bg-[#00ffff] hover:bg-[#00ffffdd] transition-all
               shadow-lg shadow-[#00ffff44]"
    onClick={onClick}
  >
    <svg className="w-8 h-8" fill="none" stroke="#0a0a1a" viewBox="0 0 24 24">
      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9 5l7 7-7 7" />
...
const PrevArrow = ({ onClick }: { onClick: () => void }) => (
  <div
    className="absolute left-4 top-1/2 -translate-y-1/2 z-10 cursor-pointer p-2
               rounded-full bg-[#00ffff] hover:bg-[#00ffffdd] transition-all
               shadow-lg shadow-[#00ffff44]"
    onClick={onClick}
  >
    ...
    <div className={`${cyberColors.glass} p-8 rounded-2xl max-w-6xl mx-auto mb-12 relative overflow-hidden border border-[#00ffff33]`}>
      ...
      <Slider {...settings}>
        {companies.map((company) => (
          <motion.div key={company.name} className="px-4 focus:outline-none">
            <div
              className="group relative h-64 cursor-pointer transform transition-all hover:scale-102"
              onClick={() => onSelectCompany(company)}
            >
              <div className={`absolute inset-0 ${cyberColors.glass} rounded-xl transition-all group-hover:border-[#00ffff]`}>
                <div className="flex flex-col items-center justify-center h-full p-8">
                  {/* Larger circular container => w-36 h-36, white background, no border */}
                  <div className="w-36 h-36 flex items-center justify-center
                                  rounded-full shadow-lg bg-white"
                  >
                    {/* Logo forced to be square & contained => maxWidth/Height 70% */}
                    <img
                      src={company.logo}
                      alt={company.name}
                      className="object-contain"
                      style={{ maxWidth: '70%', maxHeight: '70%', borderRadius: 0 }}
                    />
                  </div>
                  <h3 className={`text-xl font-bold ${cyberColors.primary} text-center mt-4`}>
                    {company.name}
                  </h3>
                  <div className={`h-1 w-16 ${cyberColors.gradient} mt-2 rounded-full transition-all group-hover:w-24`} />
                </div>
              </div>
            </div>
          </motion.div>
  ...
  return (
    <AnimatePresence mode="wait">
      ...
      >
        <div className="flex items-start gap-6 mb-8">
          <div className={`p-4 ${cyberColors.glass} rounded-xl`}>
            {/* Slightly bigger circle => w-28 h-28, white background, no border */}
          ...
          <div>
            <h2 className={`text-3xl font-bold ${cyberColors.primary} mb-2`}>
              {company.name}
            </h2>
            <p className={`${cyberColors.accent} text-lg`}>
              {company.description}
            </p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              ...
              {company.steps.map((step, index) => {
                ...
                return (
                  ...
                  >
                    ...
                    <p className={`${cyberColors.accent} text-sm`}>
                      {step.description}
                    </p>
                  </motion.div>
                );
          ...
          <div className={`${cyberColors.glass} p-6 rounded-xl col-span-full`}>
            ...
            <p className={`${cyberColors.accent} leading-relaxed`}>
...
// -----------------------------------------------------
// COMPLETE COMPANIES DATA (ALL OF THEM)
// -----------------------------------------------------
const companies: CompanyProcess[] = [
  {
    name: "Google",
